...
//                    e.printStackTrace();
        ...
        if (txnCountThe24HoursBefore == 0) txnCountPercentageChangeComparedToPreviousDay = 0;
            ...
            txnCountPercentageChangeComparedToPreviousDay = BigDecimal.valueOf((txnCountPast24Hours - txnCountThe24HoursBefore) / (double) txnCountThe24HoursBefore * 100).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();

        if (totalTxnFeesThe24HoursBefore == 0) totalTxnFeesPercentageChangeComparedToPreviousDay = 0;
        else
            totalTxnFeesPercentageChangeComparedToPreviousDay = BigDecimal.valueOf((totalTxnFeesPast24Hours - totalTxnFeesThe24HoursBefore) / (double) totalTxnFeesThe24HoursBefore * 100).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();

        if (averageTxnFeeThe24HoursBefore == 0) averageTxnFeePercentageChangeComparedToPreviousDay = 0;
        else
            averageTxnFeePercentageChangeComparedToPreviousDay = BigDecimal.valueOf((averageTxnFeePast24Hours - averageTxnFeeThe24HoursBefore) / (double) averageTxnFeeThe24HoursBefore * 100).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();

        logger.info(">>txn count last 24 hours {}", txnCountPast24Hours);
